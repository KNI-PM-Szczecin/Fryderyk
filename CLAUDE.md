...
**Every slash command requires the operator role.** A global `client.application_command_check` in `main.py` rejects any interaction whose user lacks the role from `OPERATOR_ROLE_ID` (env var, with a hardcoded default). Check failures are answered with an ephemeral message by the `on_application_command_error` handler in `main.py`. `/off` additionally carries an explicit `application_checks.has_role(...)`. When adding a new command, remember it is gated by this global check automatically.
...
- `OPERATOR_ROLE_ID` (optional, has a hardcoded default) — role required to use any slash command; read by `baseUtils.get_operator_role_id()`.
